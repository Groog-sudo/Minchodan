...
> **작성일**: 2026-06-26
> **버전**: v1.1.0 (2026-07-07 §2.4/§3 현황 각주 추가 - env var 미연동, __main__ 블록은 스모크 테스트임을 명시)
...
> **2026-07-07 현황**: 이 §2.4 작업은 아직 미착수 상태다. 실제 `vector_db_factory.get_vector_db()`와 `Retriever.__init__()`는 `CHROMA_PATH`/`CHROMA_COLLECTION` 환경변수를 읽지 않고 명시적 경로/인스턴스 인자를 받는다(`os.getenv` 호출 없음). `.env.example`에는 두 변수가 정의돼 있지만 코드에서 아직 소비되지 않는다.
...
> **2026-07-07 주의**: 아래 두 명령이 실행하는 `if __name__ == "__main__":` 블록은 현재 **자체 완결형 스모크 테스트**다. `db_builder.py`는 임시 더미 영상/디렉토리를 만들어 검증 후 `shutil.rmtree`로 정리하며, `retriever.py`는 목 임베딩으로 임시 1건짜리 DB를 만들어 검증 후 삭제한다. 즉 **현재는 `data/chroma_db`를 실제로 생성·조회하지 않는다** — §2.4가 완료되어 실제 데이터/경로를 사용하도록 리팩터링된 후에야 아래 절차가 설명하는 실제 검증이 가능하다.
