...
- **직접 충돌 위험 객체 필터 추가** (`reflex_gate.py`):
  - 무차별적인 반사 경보로 인한 피로 고갈 및 큐 지연을 막기 위해 반사 조건을 재조정함.
  - **조건 1**: 사물의 바닥(bottom_y)이 화면 최하단 5% 이내로 인접한 물체.
  - **조건 2**: 사물의 중심(center_x)이 좌우 30% 여백을 제외한 중앙 40% 충돌 회랑 내에 있는 물체.
  - 위 두 조건(극도 인접 + 정면 충돌 방향)을 모두 만족할 때만 즉각 비프음(반사)을 발생시키고, 그 외의 원경/측면 사물은 비프음 없이 800ms 뒤 부드러운 우회 가이드 음성으로만 설명하도록 차별화함.
