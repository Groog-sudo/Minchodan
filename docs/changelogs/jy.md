...
### 2026-07-23 | STT/RAG/TTS | 실기기 중복 답변 및 전화번호 발음 수정

- **커밋**: (이번 커밋)
- **원인**:
  - 단말의 서버 WAV 대기 시간이 1.2초를 넘으면 `expo-speech` 폴백이 시작됐지만, 뒤늦게 도착한 동일 이벤트의 WAV도 대기열에서 다시 재생됐습니다.
  - RAG 답변의 하이픈 전화번호를 iOS TTS와 Supertonic에 원문 그대로 전달해 숫자 묶음을 부자연스럽게 읽었습니다.
- **변경 내용**:
  - 폴백이 재생을 맡은 `event_id`를 추적하고 해당 이벤트의 늦은 WAV를 폐기해 안내가 한 번만 재생되도록 했습니다.
  - WebSocket 종료와 훅 정리 시 가이드 바이너리 타이머·이벤트 상태를 함께 초기화했습니다.
  - 표시·DB 원문은 유지하면서 `02-0000-2001` 같은 전화번호만 `공 이, 공 공 공 공, 이 공 공 일` 형태의 발화용 문자열로 변환했습니다.
  - 서버 TTS와 단말 `expo-speech` 폴백에 동일한 전화번호 발화 규칙을 적용했습니다.
- **검증 결과**:
  - `tests/test_speech_text.py`, `tests/test_ws_router_stt.py`: 12개 통과
  - `npx tsc --noEmit`: 통과
  - Ruff lint·format 및 `git diff --check`: 통과
  - 기존 `tests/test_tts_prewarm.py`의 비동기 DB fixture 오류 3건은 이번 변경과 무관한 테스트 환경 문제로 별도 확인했습니다.
...
## 2026-07-23 서울 시각장애인 기관 실데이터 RAG 전환

- 가상 기관·전화번호·주소와 개인 연락처를 제거하고 서울 지역 시각장애인 관련 기관 11곳의 공개 대표정보로 교체했습니다.
- 대상은 시각장애인복지관 5곳, 한국시각장애인연합회, 서울특별시 시각장애인 플러스 지원센터, 국립장애인도서관, 서울맹학교, 한빛맹학교, 서울시설공단 장애인콜택시입니다.
- 각 기관 문서와 서비스 문서에 공식 출처 URL, 검증일 `2026-07-23`, 검증 방법을 저장하고 RAG 검색 결과 메타데이터에도 전달하도록 변경했습니다.
- 지역번호 전화뿐 아니라 `1600-4477`, `1588-4388`, `1644-6044` 같은 전국 대표번호도 서버 TTS와 단말 음성 폴백에서 숫자별 한국어로 읽도록 보완했습니다.
- 임시 ChromaDB에서 23개 문서를 먼저 빌드하고 대표 질의 4건의 1순위 검색 결과를 검증한 뒤 운영 DB로 교체했습니다.
- 기존 운영 DB는 `data/chroma_db/convenience_guidelines.backup-20260723`에 백업했습니다.
- FastAPI를 재시작하고 `/health` 응답, 운영 컬렉션 문서 수 23개, 실제 Ollama 기반 전화번호·주소 응답을 확인했습니다.
