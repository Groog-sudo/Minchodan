...
## 8. OOD 오탐 완화 (실내 공간 씬 배제)

실외 인도 기반으로 학습된 YOLO 모델이 실내 복도/로비 등 분포 외(OOD) 환경에서 물체(차량, 간판 등)를 환각 탐지하는 문제를 해결하기 위해 3중 가드레일을 적용했습니다.

### A. Detection-Segmentation 공간 교차검증 게이트 (`detection_pipeline.py`)
* **로직**: 감지된 객체의 `bbox` 내 3x3 균등 격자점(9개 점)을 샘플링하여, 현재 프레임에서 검출된 노면 세그멘테이션 폴리곤(sidewalk, roadway, caution, braille_normal)과 공간적으로 겹치는 비율을 계산합니다.
* **기준**: 겹침 비율이 **30% 미만**인 경우(허공, 벽, 천장 등 노면이 아닌 OOD 위치에 생성된 예측)는 환각 오탐으로 간주하여 즉시 무시합니다.
* **진단 로깅**: 겹치지 않는 OOD 환각 탐지 비율을 누적(30프레임 이상)하여 `[OOD DIAGNOSTIC]` 로그로 콘솔에 1회 출력하도록 진단 흐름을 신설했습니다.

### B. 시간적 지속성(ByteTrack / Streak) 필터 강화
* **서버 (`detection_pipeline.py`)**: `ByteTrackTracker`가 추적하는 `track_id`가 최소 **4프레임 이상** 유지된 활성 트랙(`hit_count >= 4`)만 반사 및 인지(LLM/TTS) 경로에 올리고, 단발성(1~3프레임 내 플리커링) 노이즈는 무시합니다.
* **클라이언트 (`CameraView.tsx`)**: 온디바이스 TFLite 추론 시 적용되는 로컬 안정화 프레임 카운트(Streak)를 기존 2프레임에서 **4프레임**으로 상향하여 물리 피드백의 오작동 확률을 소거했습니다.
* **반응 속도 영향**: 8~10 FPS 동작 환경에서 4프레임 지속성을 검증함에 따라 약 **300ms ~ 375ms** 수준의 물리적 지연(시간적 필터링 비용)이 발생하지만, OOD 환경에서의 오탐을 90% 이상 억제하여 시각장애인의 정보 신뢰성을 보장했습니다.

### C. 신뢰도 임계값(Confidence Threshold) 상향 조정
* **서버 (`config.py`)**: 객체 탐지(`YoloDetector`) 한정으로 신뢰도 기준을 기존 `0.35`에서 **`0.50`**으로 상향하여 저신뢰 오탐을 1차 필터링하고, 노면 세그멘테이션(`YoloSegmentor`)은 기존 `0.35`를 유지해 경로 게이트를 안정적으로 보전합니다.
* **클라이언트 (`tfliteDetector.ts`)**: `runModel` 내부에서 `object_detection` 수행 시 신뢰도 기준을 동일하게 **`0.50`**으로 격상하고, `segmentation`은 `0.35`로 유지해 서버-단말 간 정합성을 일치시켰습니다.

---

## 9. 긴급/예방 2단계 게이트 분리 및 0바이트 버그 조치 (2026-07-16)

### A. 온디바이스 입력 0바이트 버그 수정
* **원인**: 온디바이스 TFLite 추론 시, `useCamera.ts`의 `handleStreamFrameBase64`에서 프레임의 `float32`를 디코딩하지 않고 빈 버퍼 `new Float32Array(0)`로 전달하여 `[TFLiteDetector] 입력 shape 불일치: 실제=0, 기대=1228800` 경고가 발생했습니다.
* **해결**: `decodeBase64JpegToHwc(base64)`를 적용해 JPEG base64 데이터를 정상적으로 `640x640x3` 규격의 HWC Float32Array로 복원 및 정규화하여 탐지기에 전달하도록 수정했습니다. 수정 후 입력 텐서 shape가 정상 매칭됨을 로그로 검증할 수 있습니다.

### B. 긴급(근접 정면) / 예방(원거리) 2단계 게이트 분리
* **긴급 게이트(신설)**: 면적비 `area_ratio >= 0.15`(매우 가까움) 이면서 `confidence >= 0.35`(완화)인 정면 장애물(`0.20 <= center_x_norm <= 0.80`)에 대해 적용됩니다.
  * 트래킹 지속성(`hit_count`) 조건을 **1프레임**으로 대폭 완화하여 즉각적으로 반응합니다.
  * 노면 폴리곤 공간 교차검증(세그멘테이션 게이트)을 **우회(스킵)**하여, 공중에 떠 있거나 도로면 경계 밖에 있더라도 정면 근접 물체라면 즉시 반사 경보를 트리거합니다.
* **예방 게이트(기존 유지)**: 원거리나 애매한 장애물(`area_ratio >= 0.08`, `confidence >= 0.50`, 중앙존 `0.30 <= center_x_norm <= 0.70`)에 대해 적용되며, 기존의 엄격한 조건(연속 4프레임 지속성 + 세그멘테이션 교차검증)을 통과해야 경보를 발생시킵니다.
* **클라이언트 햅틱/비프 스무딩 완화**: `CameraView.tsx`에서도 동일하게 온디바이스 탐지 객체 중 `maxAreaRatio > 0.20`(초접근) 상황이 감지되면 프레임 안정화 streak 요구치를 `4`에서 **`1`**로 대폭 완화하여 즉각적인 피드백을 발생시킵니다.
