...
| **`RAG_ENABLED`** | bool | 선택 | `true` | **2026-07-19 신규.** RAG 검색 on/off 스위치. `false`로 설정하면 `server/detection/consumer.py`가 RAG 검색(`retriever.search_guidance`)을 건너뛰고 L2가 순수 LLM 생성만 수행한다(`rag_context`는 "관련 수칙 없음"으로 대체). 안내 문장 어색함이 RAG 문구와 LLM 압축 지시 간 스타일 충돌 때문인지 비교 테스트하기 위해 추가 | `server/detection/consumer.py` |
