...
> **작성일**: 2026-06-27
> **수정일**: 2026-07-20
> **버전**: v0.4.33 (2026-07-20 kb 병합: 실기기 장시간 테스트 기반 데이터 정리 결함 수정 - REDIS_STREAM_MAXLEN 신규(risk.events 무제한 누적 트리밍), EVENT_FRAME_CLEANUP_INTERVAL_S 신규(기동 시 1회→주기 정리 전환). 기존 v0.4.32 이력 유지: 실외 재테스트 기반 2차 반사 억제 재조정 - REFLEX_SURFACE_SUPPRESS_TTL_S 30→60·REFLEX_SURFACE_MIN_GAP_S 15.0→45.0. 기존 v0.4.31 이력 유지: 필드 DB 분석 기반 1차 반사 억제 재조정, REFLEX_NEAR_TRACK_MIN_GAP_S 신규, jy 병합. 기존 v0.4.29 이력 유지: 코드-문서 정합 누락 변수 13종 일괄 등재, EDGE_TTS_SAMPLE_RATE 기본값 22050 정정, LLAVA_MODEL 폐기 표시)
