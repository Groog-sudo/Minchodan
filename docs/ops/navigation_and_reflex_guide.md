...
> **작성일**: 2026-07-08
> **버전**: v2.2.1 (2026-07-09 정정 노트 추가 - 팀원 원본 내용은 무변경)
> **수신**: 역할 C 개발 담당 팀원 및 백엔드 파이프라인 개발팀

> **2026-07-09 정정**: 실기기 청취 검증 결과 Piper(본 문서의 §4 배포 대상)가 흔한 음절을 발음에서
> 누락시키는 모델 자체 품질 한계가 확인되어, 인지 경로 기본 TTS 엔진이 **Supertonic 3**로
> 교체됐다(`TTS_ENGINE=supertonic`). 이 문서의 Piper 배포 절차 자체는 무효화되지 않는다 - Piper는
> `TTS_ENGINE=piper` 지정 시 쓰이는 핫스왑 폴백으로 코드가 그대로 보존되므로, 폴백 경로 점검이나
> 재배포가 필요할 때 여전히 유효한 가이드다. 상세: `docs/stage-guides/stage7_tts_design.md`,
> `docs/changelogs/kb.md`(2026-07-09).
