> **작성일**: 2026-07-13
> **버전**: v1.0.0
> **설명**: Android 실기기 서버 접속을 WiFi(평상시)와 USB(개발) 두 모드로 병행하는 현재 운영 상황 및 절차

# Android WiFi / USB 이중 접속 (평상시 vs 개발)

## 1. 배경

실기기 테스트에서 서버 WebSocket(`:8000`) 접속 주소가 환경마다 달랐다.

| 상황 | 단말 → PC 경로 | 쓰던 주소 | 한계 |
| :--- | :--- | :--- | :--- |
...
코드/설정을 매번 고쳐 빌드하지 않도록, 앱에 **WiFi / USB 토글**을 두고 두 주소를 동시에 유지한다.
...
| 파일 | 역할 |
| :--- | :--- |
| `client/src/config/index.ts` | `WIFI_HOST` / `USB_HOST` / `buildWsUrl()` / `DEFAULT_SERVER_TRANSPORT` |
| `client/src/services/serverTransport.ts` | 모드 로드·저장, 라벨 |
| `client/src/hooks/useWebSocket.ts` | `wsBaseUrl` 인자, 모드 변경 시 재연결 |
| `client/src/components/CameraView.tsx` | UI 토글 `연결: WiFi` / `연결: USB` |
...
| `EXPO_PUBLIC_NETWORK_MODE` | `lan` | `ngrok`이면 WiFi/USB 토글보다 ngrok WSS 우선 |
