...
- **해결책**:
  - 오디오 정지 신호가 수신될 때 플레이어 인스턴스를 소멸시키지 않고 **메모리에 싱글톤으로 유지**합니다.
  - 재생 시에는 비동기 `seekTo(0)`와 `pause()` 부하를 전면 제거하고, 오직 **`player.play()`만 단독 호출**하게 하여 네이티브 큐 누수 및 스레드 병목을 방지합니다.
  - 추가로, 독립된 3개의 플레이어 인스턴스를 순환 가동하는 **`3채널 오디오 풀(Audio Player Pool)` 기법**을 적용하여 개별 플레이어가 받는 호출 피로도를 1/3로 극단 축소시킵니다 (1번 플레이어가 울릴 때 2번과 3번 플레이어는 네이티브 스레드 상에서 안전하게 휴식 및 대기 완료).
