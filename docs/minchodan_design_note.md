...
> **전제:** 비전 설계서 **v1.1**(이중 경로 / YOLO26·SegFormer / cu128 / 클래스 분리)을 오버레이로 반영한다. 충돌 시 v1.1이 우선한다.
> **작성일**: 2026-06-24
> **버전**: v0.2.0
...
- **핵심 절차:**
  ...
  2. **YOLO26 Detection**(서버 기동 시 1회 로드) `predict(conf=0.35)`  클래스·bbox 파싱
  3. **SegFormer**(또는 YOLO26-Seg)  노면 의미 분할 마스크
...
- **활용 스택·핵심 함수:** Ultralytics YOLO26, SegFormer, ByteTrack, OpenCV, Redis / `YOLO(...).predict()`, `result.boxes.xyxy/conf/cls`, `tracker.update()`, `xadd()`
...
- ** v1.1 반영:**
  - 모델: YOLOv8  **YOLO26**(NMS-free, sm_120, 소형객체 최적화) + **SegFormer**(semantic seg). RT-DETR은 occlusion 백로그.
...
- **활용 스택·핵심 함수:** Ollama(Llava), OllamaEmbeddings, ChromaDB / `embed_query()`, `Chroma.from_documents()`, `imagehash.phash()` — *상용 전환 대비 `Embeddings` 추상 클래스로 랩핑*
...
- **활용 스택·핵심 함수:** ChromaDB / `similarity_search_with_score()` — *VectorDBFactory로 ChromaQdrant 추상화*
...
- **활용 스택·핵심 함수:** LangGraph, LangChain, ChatOllama / `StateGraph()`, `ainvoke()` — *LLMClientFactory(BaseChatModel)로 로컬상용 핫스왑*
...
- **활용 스택·핵심 함수:** Kokoro-82M/Coqui(서버), react-native-tts(예비), Web Audio, Haptics / `local_tts.generate()`, `decodeAudioData()` — *TTSService 추상화, 출력은 MP3/WAV로 규격 통일*
