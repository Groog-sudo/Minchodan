...
> **2026-07-28 640x640 FP16 네이티브 모델 실측 (정밀 해상도 및 30ms 속도 달성)**:
> - 원거리 소형 객체/노면 정밀 탐지를 위해 입력 해상도를 **640x640으로 원복 유지**.
> - 640x640 FP16 네이티브 TFLite 모델 및 동적 앵커/입력 버퍼 패치 적용 결과:
> - **`det` (객체 탐지)**: **30.02~31.99ms** (기존 FP32 38.9ms 대비 약 23% 최적화, 30ms대 달성)
> - **BBox 렌더링/탐지율**: `caution(0.90)`, `braille_normal`, `sidewalk_normal` 등 원거리 객체 수십 건 정밀 탐지 및 BBox 렌더링 완전 복구 확인.
> - **`total` (전처리 포함 총 지연)**: **33.24~34.93ms** (seg 미실행 프레임 기준)
