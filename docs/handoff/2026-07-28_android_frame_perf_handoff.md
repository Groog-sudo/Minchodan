...
1. **오디오 재생 중 추론 완전 중단 (최대 손실)**: 30초 중 4회(약 12.5초, 42%) 가이드 음성 재생 구간에 추론 샘플이 0건. 재생 시작(`(bytes) 재생 시작`)부터 자연 종료(`didJustFinish`)까지 추론이 멈춘다. 이것이 전체 추론을 116회/30s(3.8fps)로 끌어내린 주원인. logcat에 `MediaSession playback state 변경 1634회` / `ExpoAudio 세션 이벤트 1701회`로 오디오 세션 활동이 비정상적으로 높다. **본 최적화(백그라운드 분리)와 무관한 별도 문제** — 오디오 엔진이 JS 스레드나 handleFrame 호출을 억제하는 것으로 추정.
...
1. **오디오 재생 중 추론 중단 해소 (최우선, 최대 효과)**: 가이드 음성 재생(약 3초/회) 구간에 추론이 완전히 멈춰 전체 fps의 42%를 잃는다. 오디오 엔진이 JS 스레드/handleFrame 호출을 억제하는 구조로 보인다. 오디오 재생을 별도 처리로 분리하거나, 추론 시작 조건에서 오디오 재생 상태를 무시하도록 변경 필요. **이것만 해결해도 전체 추론이 3.8fps → 6fps 대역으로 회복**.
