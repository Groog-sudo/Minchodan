...
> 참고: Android에서도 `[CoreMLBench] ANE 가속 지연시간` 문구가 출력된다. iOS 용어가 그대로 쓰인 것으로 로그 라벨만의 문제이며 동작에는 영향 없음(정리 대상).
...
**관측**: 측정 중 서버 `server_detection`이 전부 탐지 0건으로 도착했다(`applied[srv=0 staleReject=0]`은 적용·거부 어느 카운터도 오르지 않은 상태). 온디바이스는 정상 탐지 중이었으므로 서버측 탐지 결과가 비는 원인은 별도 확인이 필요하다.
