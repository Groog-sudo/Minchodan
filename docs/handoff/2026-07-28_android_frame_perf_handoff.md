...
> **2026-07-28 416x416 FP16 네이티브 모델 실측 (iOS 성능 능가)**:
> - 416x416 FP16 TFLite 모델 재export 및 동적 앵커 디코딩 적용 결과:
> - **`det` (객체 탐지)**: **0.78~6.04ms (평균 ~2.5ms)** (iOS CoreML 6~14ms 대비 대폭 단축)
> - **`seg` (노면 분할)**: **1.00~4.01ms (평균 ~2.0ms)** (iOS CoreML 5~12ms 대비 대폭 단축)
> - **`total` (전처리 포함 총 지연)**: **5.36~14.05ms** (기존 640 모델 109.8ms 대비 약 15~20배 속도 향상)
