...
> **2026-07-29 보완(§5.15)**: 대리 지표 대신 ADPF 발열 헤드룸을 직접 읽어 캡처 간격 하한을 거는 방식으로 이 공백을 메웠다.
...
### 5.13 run/decode 분리 계측 — 후처리 병목 가설 기각 (2026-07-28)

`det_ms`는 그동안 "가속기 실행 + JVM 후처리"를 뭉쳐서 재고 있었다. 35ms 중 GPU 몫을 몰라 다음 최적화가 가속기 교체인지 후처리 개선인지 정할 수 없었으므로, `runModel()`을 `run`(delegate 실행)과 `decode`(dense head 디코드 + NMS)로 분리했다.

**측정 결과 (256 프레임, 40초, GPU FP16 640):**

| 구간 | 중앙값 | 최소 | 최대 |
| :--- | ---: | ---: | ---: |
| `prep` | 1.93ms | 0.99 | 19.15 |
| **`det_run`** | **27.70ms** | 23.72 | 47.49 |
| `det_decode` | **0.51ms** | 0.32 | 7.15 |
| `seg_run` (85회) | 34.97ms | 32.01 | 44.22 |
| `seg_decode` | 0.35ms | 0.22 | 3.96 |
| `total` | 34.83ms | 25.73 | 96.42 |

**후처리는 det 시간의 1.8%다.** 8400 앵커 x 29 클래스 = 243,600회 스트라이드 접근이 캐시 미스로 비싸리라는 가설을 세우고 클래스 우선 순차 스캔으로 재작성했으나, 재작성 후에도 0.51ms이므로 **원래부터 병목이 아니었다.** JIT과 하드웨어 프리페처가 이 정도 규모는 충분히 흡수한다.

> **결론: 남은 병목은 전부 `interpreter.run()`, 즉 가속기와 모델 자체다.** 후처리·버퍼·디코드 쪽을 더 파도 얻을 것이 1ms 미만이다. iOS 6~14ms와의 격차를 좁히려면 **INT8 양자화 + HTP(QNN/NNAPI)** 또는 **입력 해상도 축소** 외에 선택지가 없다. 다만 두 방법 모두 원거리·소형 객체(볼라드, 폴) 탐지율을 떨어뜨리므로 mAP 검증이 선행되어야 한다.

부수적으로 출력 버퍼 재사용은 적용했다(det 1.11MB + seg 1.34MB를 매 프레임 `allocateDirect` + 동일 크기 `FloatArray`로 새로 잡던 것을 제거). det 수치 자체를 유의미하게 바꾸지는 않았고 GC 압력 감소 효과이므로, 장시간 가동 안정성 관점에서만 의미가 있다.

---

### 5.14 추론 디스패치 파이프라이닝 — in-flight 1 -> 2 (2026-07-29, 실측 완료)

§5.4-1'로 등록해 둔 후보를 구현하고 실기기(Xiaomi 12, Tailscale adb)로 실측했다. **채택.**

**실측 결과 (30초, 냉각 상태)**:

| 지표 | §5.7 기준선 | 본 변경 | 판정 |
| :--- | ---: | ---: | :--- |
| **추론 디스패치** | 4.1/s | **6.33/s** (190회/30초) | **+54%** |
| `total` 중앙 | 34.83ms | **34.70ms** | 악화 없음 |
| `total` p90 | 69.6ms | **71.30ms** | 큐 적체 없음 |
| `det_run` 중앙 | 27.70ms | **27.61ms** | 동일 |
| 서버 `detection 수신` | 242건/30초 | **243건/30초** | 8.1fps 유지 |
| 동적 FPS 조절 발생 | 0건 | **0건** | 진동 없음 |

핵심은 **p90이 69.6 -> 71.3ms로 유지**된 점이다. in-flight를 늘리면 큐 대기가 지연으로 전가될 것을 우려했으나(검증 항목 ②), 실제로는 가속기가 놀던 구간만 메워졌다.

목표 8/s에 못 미친 6.33/s인 이유는 상한이 **프레임 공급 쪽으로 넘어갔기** 때문이다. `handleFrame`은 8.4~9.0/s(약 115~138ms)인데 `REAL_DETECT_MIN_INTERVAL_MS`가 120ms라, 도착 지터로 간격이 120ms에 못 미치는 프레임이 주기적으로 걸러진다.

**`REAL_DETECT_MIN_INTERVAL_MS` A/B (2026-07-29) — 120ms 유지**

| 값 | 디스패치/30초 | 서버 수신/30초 | `total` 중앙 | SoC |
| :--- | ---: | ---: | ---: | ---: |
| **120ms (현행)** | **194** | 247 | 33.72ms | 상승 중 |
| 110ms | 196 | 250 | 43.83ms | 71.8°C |
| 90ms | 미측정 | – | – | – |

**110ms는 이득이 없다(194 -> 196, 1%).** 열 상태가 두 창에서 달라 엄밀한 대조는 아니지만, 두 값 모두 약 6.5/s로 수렴한 것은 **디스패치 상한이 이미 `minInterval`이 아니라 프레임 공급·도착 지터 쪽에 있음**을 시사한다. 90ms 결정 실험은 단말 수신 전화로 중단했다. **상수는 120ms로 되돌렸다.**

더 올리려면 `minInterval`이 아니라 프레임 공급(플러그인 콜백 138ms, §5.4-2)을 손대야 한다.

**문제**: `handleFrame`은 8.4~9.0fps인데 추론 디스패치는 4.1/s에 머물렀다. `detectingRef`(boolean)가 in-flight를 1개로 묶어, 다음 디스패치가 "직전 추론 완료(약 110ms) 이후 + `REAL_DETECT_MIN_INTERVAL_MS`(120ms) 경과"를 모두 만족해야 했기 때문이다(실효 약 240ms). 반면 §5.13 실측에서 네이티브 추론 자체는 `total` 34.8ms이고 나머지 약 75ms는 브릿지 왕복·JS 스케줄링이므로, **가속기는 주기의 대부분을 유휴 상태로 보내고 있었다.**

**변경 요약**: `detectingRef`(boolean) -> `inFlightRef`(카운터, 상한 `MAX_INFLIGHT_DETECTIONS=2`).

| 항목 | 내용 |
| :--- | :--- |
| 상한을 2로 둔 이유 | 네이티브는 단일 `TFLiteInference` HandlerThread라 3 이상은 큐 적체(결과 지연)만 늘린다. 반사 경로는 처리량보다 신선도가 중요하므로 "실행 1 + 대기 1"이 상한이다 |
| 폴백 경로 제외 | `requiresFloat32=true`(JS JPEG 디코드)와 Mock은 추론이 JS 스레드를 실제로 점유하므로 in-flight 1을 유지한다 |
| 프레임 신선도 | 픽셀은 디스패치 시점 `ReflexFrameCache.snapshot()`으로 확정되므로 대기 중인 작업이 다른 프레임으로 바뀌지 않는다 |
| 결과 순서 | HandlerThread 순차 실행이라 순서대로 오는 것이 정상이나, `runDetectionResult` 입구에서 `detectTs < lastAppliedDetectTs`인 결과를 버려 오래된 BBox·경보가 최신 판정을 덮지 않게 한다 |
| 네이티브 안전성 | `scratchFloats`/`inputBuffer`/`OutputScratch` 재사용은 단일 스레드 전제인데, 큐잉이 늘 뿐 동시 실행은 없으므로 브릿지는 무변경이다 |

**성격 주의**: 이 개선은 **처리량**이다. 개별 추론 지연(약 110ms)은 줄지 않으므로 반사 경보의 지연이 아니라 "건너뛰던 프레임"이 줄어든다.

---

### 5.15 발열 대응 — ADPF는 이 단말에서 무효, 캡처 지연 실측으로 대체 (2026-07-29)

§5.8의 "남은 설계 공백"을 메웠다. 다만 **1차 선택지였던 ADPF는 Xiaomi 12에서 사용할 수 없음이 실기기로 확인됐다.**

```bash
adb shell cmd thermalservice headroom 10
# Headroom in 10 seconds: NaN
```

`PowerManager.getThermalHeadroom`이 NaN을 반환한다(Snapdragon 8 Gen 1 / HyperOS가 미구현). `getCurrentThermalStatus`도 §5.8 실측에서 스로틀링 중 0(NONE)이었다. **이 단말에는 쓸 수 있는 발열 API가 없다.** 앱 로그로도 확인된다.

```
[Camera] 발열 헤드룸 지원 여부: headroom=미지원 thermalStatus=0 (헤드룸 미지원(NaN))
```

ADPF 경로(`ThermalBridgeModule.kt`, `services/thermalBridge.ts`)는 **코드에 유지**한다. 지원 단말에서는 동작하고, 미지원이면 자동 비활성이며 비용이 0이다. iOS는 브릿지 자체가 없어 폴링을 걸지 않는다.

**대체 신호**: §5.8이 원래 제안했던 안(`handleFrame` 실측 간격을 컨트롤러 입력으로)을 구현했다. 스로틀링의 최종 증상은 카메라 세션 fps 붕괴(§5.8: 30 -> 중앙 12.6)이므로, **프레임 도착 간격이 지시 간격보다 크게 벌어지면 원인이 무엇이든 물러선다.** 발열 API가 유효한 단말에서는 두 신호 중 더 보수적인(큰) 하한이 적용된다.

| 신호 | 하한 상승 | 하한 해제 |
| :--- | :--- | :--- |
| ADPF headroom (지원 단말) | >= 0.85 -> 175ms, >= 1.0 -> 200ms | <= 0.7 |
| 캡처 도착 간격 / 지시 간격 | >= 1.4배 -> 175ms, >= 1.8배 -> 200ms | < 1.15배 + 20초 dwell |

중간 구간은 히스테리시스로 직전 하한을 유지한다. 하한이 **올라가면 즉시** 반영하고(발열은 기다릴수록 나빠진다), **내려갈 때는 하한만 낮춘다** — 실제 복귀는 기존 회복 분기가 20ms씩 점진 수행하므로, 서버 백프레셔나 추론 과부하로 올라가 있던 간격을 발열 회복이 한 번에 밀어내지 않는다.

**구현**: `ThermalBridgeModule.kt`(신규), `services/thermalBridge.ts`(신규), `useCamera.ts`(`thermalFloorRef` + `captureFloorRef`, 실효 하한은 둘 중 큰 값). 도착 간격 관측은 캡처 진입점 `handleStreamFrameBase64`에서 하며 비용은 배열 push + 5초에 한 번의 24개 정렬이다.

---

### 5.16 발열 스로틀링 실시간 재현 (2026-07-29) — 임계 재검토 근거

§5.14 측정 중 연속 가동 약 25분(배터리 구동, USB 미연결) 시점에 §5.8이 그대로 재현됐다. 30초 창 3회를 연속 수집한 결과다.

| 창 | 디스패치/30초 | `total` 중앙 | `total` p90 | 서버 수신/30초 |
| :--- | ---: | ---: | ---: | ---: |
| 1 (냉각) | **190** | **34.70ms** | 71.30ms | **243** (8.1fps) |
| 2 | 143 | 41.06ms | 77.09ms | 204 (6.8fps) |
| 3 | 123 | **60.99ms** | 103.91ms | 198 (6.6fps) |

창 3 시점의 하드웨어 상태:

| 측정 | 값 | 비고 |
| :--- | ---: | :--- |
| CPU thermal zone (`cpu-1-6`) | **69.1°C** | §5.8 스로틀 구간(67~70°C)과 일치 |
| cpu7 주파수 | **1.29GHz** | 정상 약 3.0GHz |
| **배터리 온도** | **32.2°C** | **정상값** |

> **주의: 배터리 온도로 발열을 판단하면 안 된다.** 배터리 32.2°C는 완전히 정상인데 SoC는 69°C로 스로틀 중이었다. USB 미연결 조건에서는 배터리가 데워지지 않아 특히 그렇다. §5.8은 충전 중(44.3°C) 측정이라 이 함정이 드러나지 않았다. **`/sys/class/thermal/thermal_zone*/temp`의 `cpu-*` 존을 직접 읽을 것.**

탐지를 끄고 4분 유휴로 두자 69.1°C -> **40.7°C**로 회복했다. 발열 요인이 실재함을 확인했다.

**임계 재검토 근거**: 이 재현 구간에서 §5.15의 캡처 하한은 **발동하지 않았다.** 도착 간격이 약 151ms / 지시 125ms = **1.21배**로 상승 임계 1.4배에 못 미쳤기 때문이다. 성능은 이미 total 34.7 -> 61.0ms(76% 악화)였다. 두 가지 해석이 가능하다.

1. 임계 1.4가 너무 느슨하다 -> 1.25 근처로 낮춰야 한다.
2. 1.2배 수준의 완만한 저하에서 캡처를 더 줄이는 것은 오히려 손해다(프레임만 줄고 열은 크게 안 준다).

**어느 쪽인지는 후퇴가 실제로 냉각으로 이어지는지 측정해야 정해진다.** 하한을 강제로 200ms에 고정한 채 온도 곡선을 10분 이상 추적하는 실험이 필요하며, 이번 세션에서는 수행하지 못했다(단말 잠금으로 냉각 대조 측정 중단). **임계값을 근거 없이 조정하지 말 것.**
