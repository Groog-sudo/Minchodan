...
- **자바스크립트 중계 (`CameraView.tsx`)**:
  - 프로미스로부터 수신된 `benchmark` 객체를 디코딩하여 메트로 번들러 콘솔 로그로 즉시 출력합니다. 접두사는 2026-07-28부터 플랫폼 중립 `[OnDeviceBench]`이며 실제 엔진명을 함께 찍습니다(기존 `[CoreMLBench] ANE 가속`은 Android에서 부정확했습니다).
