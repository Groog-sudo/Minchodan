...
본 문서는 Minchodan 7단계 파이프라인 중 **3단계 (AI 장애물 실시간 인식)** 의 백엔드 FastAPI 구현을 위한 상세 설계서입니다. 팀원이 **Yolo 26N - Object Detection / Yolo 26N - Segmentation 모델 학습 중**이라는 전제하에, 가중치 도착 전까지 **Mock Detector + 인터페이스 추상화** 패턴으로 구현하여 팀원 가중치 도착 시 교체만으로 즉시 작동하도록 합니다.

### 1.1 3단계 정체성

2단계(프레임 수신)에서 받은 640x640 BGR 프레임을 **Yolo 26N - Object Detection**으로 추론하여 킥보드, 볼라드, 계단, 차량 등 위험 사물을 탐지하고, **Yolo 26N - Segmentation**으로 노면 상태(보행 가능 영역, 횡단보도, 점자블록 파손 등)를 분할하며, **ByteTrack**으로 Track ID를 부여합니다. **이중 게이트**(Reflex Gate + Surface Gate) 룰로 위험도를 1차 분류하여 반사 경로(즉시 경보)와 인지 경로(상세 가이드)로 분기합니다.
...
| **인지** | mid/low | Detection+Seg → ByteTrack → `xadd("risk.events")` → LangGraph L1/L2/L3 + RAG → 실시간 TTS | 실시간 합성 상세 가이드 | 1~2Hz |

---

## 2. 구현 범위

팀원 모델 학습 중이라는 전제와 사용자 승인에 따라, 본 설계서의 구현 범위는 **`server/detection/` + `server/bus/`** 로 한정합니다. 1·2단계는 인터페이스만 맞추고, 가중치는 Mock 폴백으로 대체합니다.
...
| `server/detection/mock_detector.py` | Mock 구현 | `MockDetector`, `MockSegmentor` — 가중치 없을 때 가짜 탐지 결과 반환 (guide 17.2 Mock 폴백) |
| `server/detection/yolo_detector.py` | Yolo 26N Detection | `YoloDetector` — `ultralytics.YOLO` 로드 + `predict(conf=0.35)` + boxes 파싱. 로드 실패 시 Mock 폴백 |
...
| 파일 | 변경 내용 |
| --- | --- |
| `.env.example` | v1.1 기준 통일: `YOLO26_WEIGHTS` → `YOLO26N_OBJECT_DET=server/models/yolo26n/object_detection.pt`, `SEGFORMER_WEIGHTS` → `YOLO26N_SEG=server/models/yolo26n/segmentation.pt`. SegFormer 잔재 제거 |
| `requirements.txt` | `pytest` 추가 (테스트 검증용) |
...
팀원이 모델 학습 중이므로 가중치 파일이 없는 상태에서도 파이프라인 전체 흐름을 검증할 수 있도록 **인터페이스 추상화 + Mock 폴백** 패턴을 적용합니다.
...
`DetectionPipeline`은 `DetectorInterface`와 `SegmentorInterface`에 의존하므로, 가중치 도착 후 `MockDetector` → `YoloDetector`로 교체만 하면 작동합니다. `config.py`의 팩토리 함수가 환경 변수와 파일 존재 여부로 적절한 구현체를 반환합니다.
...
> **참고**: `model.track()`은 `YoloDetector` 내부에서만 사용 가능하므로, `ByteTrackTracker`는 `YoloDetector`의 추론 결과(`result.boxes.id`)를 받아 track_id를 파싱하는 래퍼로 설계합니다. `MockDetector` 사용 시 track_id는 None으로 처리됩니다.
...
| 모델 파일 없음 | `MockDetector`로 폴백, 로그 경고 | Mock 폴백 (17.2 #3) |
...
async def run(
    ...
    stream: str,              # "reflex" | "cognitive" — 2단계 ProcessedFrame.stream 호환
...
| `crosswalk` | 횡단보도 | Surface Gate (P0) | high |
| `roadway` | 차도 | (주의) | mid |
...
| `surface_crosswalk` | front | 횡단보도 하단 검출 | Surface Gate |
...
| TC-DET-002 | track_id 부여 | ByteTrack `update()` → track_id | Mock + ByteTrack 래퍼 | 대기 |
...
| TC-DET-005 | Reflex Gate 분기 | 고위험+근접 → `alert_id`+방향 | 룰베이스 테스트 (Mock 결과 주입) | 대기 |
| TC-DET-006 | Surface Gate 분기 | P0 노면 하단 검출 → `alert_id` | 룰베이스 테스트 | 대기 |
| TC-DET-007 | Redis 컨텍스트 TTL | 30초 후 Track ctx 키 자동 삭제 | Redis 가드레일 테스트 | 대기 |
| TC-DET-008 | mid/low 발행 | `xadd("risk.events")` 정상 | `xadd` 호출 검증 | 대기 |
| TC-DET-009 | 무탐지 빈 리스트 | 에러 없이 빈 리스트 반환 | None 프레임 입력 | 대기 |
...
| Mock 폴백 | 17.2 #3 | `config.py`, `mock_detector.py` | 가중치 없을 시 Mock 구현체 사용 |
...
| 상황 | 처리 | 적용 파일 |
| --- | --- | --- |
| 모델 파일 없음 | 서버 시작 실패 + 로그 경고, Mock 폴백 | `config.py`, `yolo_detector.py` |
| 프레임이 None | 추론 스킵, 빈 detections 반환 | `detection_pipeline.py` |
| Redis 연결 실패 | 컨텍스트 업데이트 스킵, 탐지 결과는 정상 반환 | `redis_client.py` |
| CUDA OOM | CPU 폴백 (`device='cpu'`) | `yolo_detector.py`, `yolo_segmentor.py` |
