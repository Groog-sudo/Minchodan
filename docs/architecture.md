...
graph TD
    ...
    subgraph Server ["GPU Server (FastAPI)"]
        ...
        subgraph Detection ["3. 탐지·분할·게이트"]
            YOLO["YOLO26 Detection"]
            Seg["SegFormer Segmentation"]
    ...
    subgraph Data ["Data & Storage"]
        ...
        Models[("models/<br/>YOLO26·SegFormer 가중치")]
    end
...
| `server/detection/yolo_detector.py` | YOLO26 `predict(conf=0.35)`, boxes 파싱 | 3 |
| `server/detection/segformer_segmentor.py` | SegFormer 의미분할 마스크 생성 | 3 |
...
| `server/models/yolo26/` | YOLO26 Detection 가중치 (git-ignore) | 3 |
| `server/models/segformer/` | SegFormer 체크포인트 (git-ignore) | 3 |
...
2. **YOLO26 Detection** `predict(conf=0.35)`  클래스·bbox 파싱
3. **SegFormer**  노면 의미 분할 마스크
...
sequenceDiagram
    ...
    WS->>Det: 프레임 디코딩
    Det->>Det: YOLO26 + SegFormer + ByteTrack
    alt high 위험 (반사)
