...
from seg_config import SEG_CLASS_TO_ID, SEG_CLASS_ALIASES, SEG_DATASET_ROOT
...
def extract_seg_label(raw_label: str) -> str | None:
    ...
    if "floor_normal" in raw_label or "sidewalk" in raw_label or "보도" in raw_label or "인도" in raw_label:
    ...
    if "road" in raw_label or "차도" in raw_label or "도로" in raw_label:
        return "roadway"
    if "damage" in raw_label or "파손" in raw_label or "턱" in raw_label or "경사로" in raw_label or "웅덩이" in raw_label or "공사" in raw_label or "curb" in raw_label or "계단" in raw_label or "stairs" in raw_label or "step" in raw_label:
        return "caution"
        
    return None

def process_aihub_seg_json(json_path: Path, image_dir: Path, label_dir: Path, hash_index: HashIndex) -> int:
    ...
    for ann in annotations:
        ...
        if isinstance(segmentation[0], list):
            for point in segmentation:
                if len(point) >= 2:
                    flat_coords.extend([float(point[0]) / float(width), float(point[1]) / float(height)])
        else:
            for i in range(0, len(segmentation), 2):
                flat_coords.extend([float(segmentation[i]) / float(width), float(segmentation[i+1]) / float(height)])
...
def main() -> None:
    ...
    parser.add_argument("input_dir", type=Path, help="Directory containing raw AI Hub JSON and images")
