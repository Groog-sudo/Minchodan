...
def build_parser() -> argparse.ArgumentParser:
    ...
    fiftyone.add_argument("--dataset", choices=["open-images-v7", "coco-2017"], default="open-images-v7")
    ...
    fiftyone.add_argument("--only-class", action="append", default=[], help="Target class name to download. Repeatable.")
    ...
    collect_weak = subparsers.add_parser("collect-weak", help="API-only collection for weak classes.")
    ...
    scrape_class = subparsers.add_parser("scrape-class", help="Crawl all curated queries for a target class.")
    ...
    label.add_argument("--source", type=Path, default=SETTINGS.raw_root, help="Raw image directory.")
    label.add_argument("--model", default=SETTINGS.default_model, help="Ultralytics YOLO 26n model path/name.")
    label.add_argument("--confidence", type=float, default=SETTINGS.min_confidence)
    label.add_argument("--only-class", action="append", default=[], help="Keep only the given class id/name. Repeatable.")
...
def main() -> None:
    ...
    if args.command == "kaggle":
        ...
        AutoLabeler(model_name=args.model, confidence=args.confidence, allowed_classes=allowed_classes).label_directory(args.source)
    elif args.command == "plan":
        specs = [get_class_spec(args.target_class)] if args.target_class else list(CLASS_SPECS.values())
        print_plan(specs)
        if args.export:
