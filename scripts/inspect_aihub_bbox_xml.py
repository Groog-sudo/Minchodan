...
def build_summary(
    ...
    candidates = [
        image for image in images if any(box.label in target_labels for box in image.boxes)
    ]
    candidates.sort(
    ...
    return {
        ...
        "top_candidates": [
            image_to_dict(image, target_labels) for image in candidates[: max(0, top_k)]
        ],
    }
...
def render_report(summary: dict[str, Any]) -> str:
    ...
    label_rows = [[label, count] for label, count in summary["counts"]["label_counts"].items()]
    candidate_rows = [
        [
            item["name"],
            ...
            [
                ...
                markdown_table(
                    ...
                    [
                        ...
                        [
                            "이번 주 사용",
                            "바운딩박스 폴더에서 이미지 1~3장만 골라 YOLO 데모 입력으로 사용합니다.",
                        ],
...
def main() -> None:
    ...
    target_labels = {
        normalize_label(label) for label in args.target_labels.split(",") if label.strip()
    }
