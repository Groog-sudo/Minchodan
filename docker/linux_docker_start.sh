...
###############################################################################
# DoctorSkin Docker Build and Start - Linux
#
...
# 7. Linux 데스크톱 환경의 xdg-open/gio 명령으로 브라우저를 엽니다.
#
# 사용 예:
#   chmod +x docker_start_v03_linux.sh
#   ./docker_start_v03_linux.sh
###############################################################################
...
is_web_port_open() {
  ...
  # Linux 배포판마다 설치된 도구가 다를 수 있으므로 python3, nc, curl 순서로 시도합니다.
  local port="$1"
...
open_url() {
  # 대부분의 Linux 데스크톱 환경에서는 xdg-open이 기본 브라우저를 엽니다.
  # 일부 GNOME 계열 환경에는 gio open이 더 안정적인 경우가 있어 fallback으로 둡니다.
  local url="$1"
  ...
  if command -v xdg-open >/dev/null 2>&1; then
    ...
    echo "[WARN] Could not find xdg-open or gio. Please open the URL manually."
  fi
}
