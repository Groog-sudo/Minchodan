...
| 콘솔 관리자 로그인이 항상 401, `admin_accounts` 조회 시 0건 | DB | `docker compose -f docker/<compose file>`처럼 `--env-file`을 생략하면 Compose가 컴포즈 파일이 있는 `docker/`를 프로젝트 디렉토리로 잡아 그 안에 없는 `.env`를 찾는다. 루트 `.env`의 팀 공동 Tailscale `DB_HOST`가 전혀 반영되지 않고 `${DB_HOST:-mariadb}`가 로컬 빈 DB로 조용히 폴백된다(2026-07-19 실측: `docker compose config`로 `DB_HOST: mariadb` 확인, force-recreate로도 재발) | 항상 저장소 루트에서 `docker compose --env-file .env -f docker/<compose file> ...`로 실행. 확인은 `docker compose config`를 필터 없이 출력하지 말고(`DB_PASSWORD` 평문 노출 위험) `exec fastapi sh -c 'echo "DB_HOST=$DB_HOST"'`로 특정 키만 |
| YOLO 추론이 프레임마다 `'Conv' object has no attribute 'bn'`로 실패, 로그에 매번 `AutoUpdate success` 직후 발생 | 서버 추론 | ultralytics가 모델 로드/추론 시 체크포인트에 내장된 requirements를 현재 설치본과 비교해 불일치하면 `AUTOINSTALL`(기본 True, `YOLO_AUTOINSTALL` 환경변수) 설정에 따라 런타임에 조용히 `pip install`한다. 방금 갱신된 디스크 상 패키지와 이미 임포트된 모듈 상태가 어긋나며 매 프레임 재발(2026-07-19 실측) | `docker-compose.macos.yml`의 fastapi `environment:`에 `YOLO_AUTOINSTALL=False` 추가 후 `docker compose --env-file .env -f docker/docker-compose.macos.yml up -d --force-recreate fastapi`(이미지 재빌드 불필요, 환경변수만 주입) |
