...
3. "[회피 힌트]"는 완성 문장이 아닌 행동 조각이다. 힌트에 방향 표현이 있어도 무시하고, 방향은 오직 [탐지 방향]만 사용한다.
4. "N시 방향 [장애물명] 주의" 또는 "[장애물명] 발견, N시로 우회" 패턴을 주로 사용하되, 필요하면 회피 힌트 키워드를 짧게 조합한다.
5. 존댓말 (~하세요, ~세요) 사용
6. "정지", "멈추세요", "천천히 멈추세요", "대기하세요" 등 중복된 정지 명령 표현 절대 금지
...
async def l2_generator_node(state: dict) -> dict:
    ...
    # 2026-07-20: [안전 수칙] 완성문 대신 [회피 힌트] 행동 조각을 주입한다.
    hint_str = rag_context if rag_context and rag_context != "관련 수칙 없음" else ""
    hint_block = f"[회피 힌트]: {hint_str}\n" if hint_str else "[회피 힌트]: 주의하며 우회\n"
    user_prompt = (
