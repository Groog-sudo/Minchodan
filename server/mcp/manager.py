...
class MCPManager:
    ...
    def register_listener(self) -> asyncio.Queue:
        ...
        logger.info(f"[MCP MANAGER] 새로운 모니터링 리스너 등록됨. 총 리스너: {len(self.listeners)}")
        return queue
    ...
    async def broadcast_event(self, event_type: str, payload: Dict[str, Any]):
        ...
        for queue in self.listeners:
            try:
                ...
                logger.error(f"[MCP MANAGER] 리스너 브로드캐스트 중 예외: {str(e)}")
                stale_queues.append(queue)
                
        for sq in stale_queues:
            self.unregister_listener(sq)

    async def start_consumer(self, redis_url: str = None):
        """
        ...
        try:
            ...
            logger.error(f"[MCP MANAGER] Redis Connection Failed: {str(e)}")
            return
        ...
        async def _consume_loop():
            ...
            while True:
                try:
                    # xread 블로킹 1초
                    streams = await self._redis_client.xread({self.stream_key: last_id}, count=10, block=1000)
                    if streams:
                    ...
                    logger.error(f"[MCP MANAGER] Stream 읽기 예외: {str(e)}")
