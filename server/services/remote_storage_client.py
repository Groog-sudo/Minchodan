...
def _timeout_seconds() -> float:
    return float(_first_env("IMAGE_UPLOAD_TIMEOUT_SECONDS", "EVENT_FRAME_UPLOAD_TIMEOUT_SEC") or "3")
...
async def upload_event_frame(event_id: str, jpeg_bytes: bytes) -> RemoteStoreResult:
    ...
    if error_code:
        logger.error("[RemoteStorage] 이벤트 프레임 업로드 실패: event_id=%s, error=%s", event_id, error_code)
        return RemoteStoreResult(
            object_key=None,
...
async def upload_stt_audio(
    ...
    if error_code:
        logger.error("[RemoteStorage] STT 오디오 업로드 실패: event_id=%s, error=%s", event_id, error_code)
        return RemoteStoreResult(
            object_key=None,
            status="upload_failed",
            error_code=error_code,
