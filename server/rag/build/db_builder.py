...
def build_database(video_path: str, output_dir: str, db_persist_dir: str, embeddings: Embeddings, force_mock_captioner: bool = False) -> Chroma:
    """
    ...
    if not os.path.exists(parent_dir):
        try:
            ...
            raise PermissionError(f"디스크 쓰기 권한이 없거나 경로가 잘못되었습니다: {parent_dir}. 에러: {e}")
    ...
    for i, frame_path in enumerate(unique_frames):
        ...
        if force_mock_captioner:
            # 캡션 Mocking
            if i % 3 == 0:
                caption = f"길가 한가운데 전동 킥보드가 쓰러져 있고 통행을 방해하는 화면입니다."
                scene_type = KICKBOARD
                risk_level = "mid"
                objects = [KICKBOARD]
            elif i % 3 == 1:
                caption = f"화강암 재질의 볼라드가 인도 보도블록 위에 불쑥 솟아 있는 모습입니다."
                scene_type = BOLLARD
                risk_level = "mid"
                objects = [BOLLARD]
            else:
                caption = f"파편이 깨지고 마모되어 형태가 어지러운 노란색 파손 점자블록이 보입니다."
        ...
        metadata = {
            ...
            "objects": json.dumps(objects),  # ChromaDB 메타데이터는 단순 기본 타입 또는 문자열이어야 함
