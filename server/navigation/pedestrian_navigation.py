...
def search_poi(app_key, keyword, role_name="목적지"):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            while True:
                try:
                    choice = input(f"👉 원하는 {role_name} 번호를 선택하세요 (1~{len(pois)}): ").strip()
                    ...
                    if 0 <= selected_idx < len(pois):
                        ...
                        print(f"범위를 벗어났습니다. 1부터 {len(pois)} 사이의 숫자를 입력해 주세요.")
                except ValueError:
...
def simulate_realtime_walking(route_data):
    ...
    for i, current_pos in enumerate(path_coordinates):
        ...
        print(f"🚶 [보행 중] 현재 좌표: ({curr_lat:.5f}, {curr_lon:.5f}) | 목적지까지 남은 거리: {remaining_route_distance:.1f}m")
