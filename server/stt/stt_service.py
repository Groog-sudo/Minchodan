...
class SttService:
	...
	@classmethod
	def transcribe_file(cls, saved_path: Path, model_name: str | None = None) -> SttTranscribeResult:
		"""
		[바이브 코딩 부분]
		운영 STT 실행 진입점.
	...
	def _transcribe_production(cls, saved_path: Path, model_name: str) -> SttTranscribeResult:
		...
		try:
			...
			raise RuntimeError(f"STT transcribe failed: file={saved_path.name}, model={model_name}") from exc
