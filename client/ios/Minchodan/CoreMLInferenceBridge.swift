// 미사용 사본 (Xcode 빌드 미대상): project.pbxproj의 "Minchodan" 그룹에 path 속성이
// 없어서 실제 빌드 타겟은 client/ios/CoreMLInferenceBridge.swift(프로젝트 루트)로 resolve된다.
// 이 파일은 프로젝트에 실제로 연결되어 있지 않다 (.d 의존성 파일로 확인함, 2026-07-06).
